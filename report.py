...
class CsvWorker:
    ...
    def csv_ﬁler(self, vacancy_in, fields):
        salary = Salary(vacancy_in[fields.index("salary_from")], vacancy_in[fields.index("salary_to")], vacancy_in[fields.index("salary_currency")])
        vacancy = Vacancy(vacancy_in[fields.index("name")], salary, vacancy_in[fields.index("area_name")], vacancy_in[fields.index("published_at")])
        return vacancy
...
class HtmlGenerator:
    ...
    def generate_html(self, dicts, image_path, prof_name):
        html =  """<!DOCTYPE html>
                    ...
                    <img src=\""""+image_path+"""\">"""
        #1
        titles = ["Год", "Средняя зарплата", "Средняя зарплата - " + prof_name, "Количество вакансий", "Количество вакансий - " + prof_name]
...
class Report:
    ...
    def generate_graph(self, dicts, prof_name):
        ...
        plt.subplot (2, 2, 4)
        plt.pie(list(dictsCities[1].values()) + [1 - sum(list(dictsCities[1].values()))], labels=list(dictsCities[1].keys())+["Другие"])
...
def get_data(vacancies_objects, prof_name):
    ...
    for vacancy in vacancies_objects:
        avg_salary = (vacancy.salary.salary_from + vacancy.salary.salary_to) / 2 * currency_to_rub[vacancy.salary.salary_currency]
